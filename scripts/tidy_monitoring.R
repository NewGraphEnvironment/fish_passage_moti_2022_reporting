...
form_names <- list.files(path = paste0('../../gis/mergin/',
           ...
           pattern = glob2rx(paste0('form_', form_type, '*.gpkg')),
           full.names = T
           )

# this step only makes sense if we are amalgamating more than one form and want to visualize everything together.
form_prep <- form_names %>%
  ...
  # !!!!!!!project specific
  filter(
    str_detect(stream_name, "Bittner|Cross")
  ) %>%
  mutate(pscis_crossing_id = case_when(str_detect(stream_name, "Cross") ~ 198283,
                                       T ~ pscis_crossing_id)) %>%
  #burn to csv to use in monitoring summary
  readr::write_csv(paste0(
    'data/dff/monitoring_form_all_',
    format(lubridate::now(), "%Y%m%d"),
    '.csv'), na = '')
# burn out if we have more than one input form and want to visualize (could put if statement)
...
form_prep1 <- form_prep %>%
  ...
  # needed to hash out because we have an old form version here
  # !!!!!!!!!!!!!!rehash with form containing date_time_start
...
# remove our non-sites, make custom tweaks and burn to a csv
# utms are wrong but will caress by hand.
form %>%
  #sort data by date
  arrange(date) %>%

  readr::write_csv(paste0(
    'data/dff/pscis_form_tidy_',
    format(lubridate::now(), "%Y%m%d"),
    '.csv'), na = '')

# --------------------five mile creek--------------------
#pain but need to do seperate bc forms are different
# name the project directory we are pulling from
dir_project <- 'bcfishpass_skeena_20220823'
...
form1 <- "../../gis/mergin/bcfishpass_skeena_20220823/form_pscis_202209061414.gpkg"
...
form_prep1 <- form_prep %>%
  ...
  # !!!!!!!!!!!!!!rehash with form containing date_time_start
  dplyr::mutate(
    # date = lubridate::ymd_hms(date_time_start),
                date_time_start = lubridate::ymd_hms(date_time_start),
                survey_date = lubridate::date(date_time_start),
                time = hms::as_hms(date_time_start),
                date =  as_date(date)) %>%
